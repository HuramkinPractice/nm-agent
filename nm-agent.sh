...
if [ -e /etc/nodemonitor/nm-data.log ]
then
	data=($(cat /etc/nodemonitor/nm-data.log))
...
# System load cache
echo "$time $cpu $io $idle $rx $tx" > /etc/nodemonitor/nm-data.log
...
# Get network latency
ping_eu=$(prep $(num "$(ping -c 2 -w 2 1.1.1.1 | grep rtt | cut -d'/' -f4 | awk '{ print $3 }')"))
ping_us=$(prep $(num "$(ping -c 2 -w 2 8.8.8.8 | grep rtt | cut -d'/' -f4 | awk '{ print $3 }')"))
ping_as=$(prep $(num "$(ping -c 2 -w 2 8.8.4.4 | grep rtt | cut -d'/' -f4 | awk '{ print $3 }')"))
